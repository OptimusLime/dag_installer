...
const selectDefaultPlatform = () => {
    const platform = os.platform()
    if (platform == "darwin")
        return "Xcode"
    else if (platform == "win32" || platform == "cygwin")
...
class LS extends Command {
    async run() {
        ...
        const CmakeGen = flags.generator || selectDefaultPlatform()
        this.log("Graph", graph)
        this.log("Install", sortedInstall)
        this.log("OS Platform: ", os.platform())
        this.log("CMake Generator: ", CmakeGen)
        for (var g = 0; g < sortedInstall.length; g++) {
            ...
            if (shell.exec(`npm run primeinstall -- -CG "${CmakeGen}"`).code !== 0) {
        ...
        this.log("Finished compiling dependencies. Building main project? ", flags.main)
        if (flags.main) {
            shell.cd(`${rdir}`);
            // then execute the main install 
            if (shell.exec(`npm run primeinstall -- -CG "${CmakeGen}"`).code !== 0) {
                shell.echo(`Error: prime install failed ${depName}`);
                shell.exit(1);
            }
